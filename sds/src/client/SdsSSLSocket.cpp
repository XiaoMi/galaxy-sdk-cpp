...
// SdsSSLSocket implementation
SdsSSLSocket::SdsSSLSocket(boost::shared_ptr<SSLContext> ctx):
  TSocket(), server_(false), ssl_(NULL), ctx_(ctx) {
}

SdsSSLSocket::SdsSSLSocket(boost::shared_ptr<SSLContext> ctx, THRIFT_SOCKET socket):
  TSocket(socket), server_(false), ssl_(NULL), ctx_(ctx) {
}

SdsSSLSocket::SdsSSLSocket(boost::shared_ptr<SSLContext> ctx, string host, int port):
  TSocket(host, port), server_(false), ssl_(NULL), ctx_(ctx) {
}

SdsSSLSocket::~SdsSSLSocket() {
  close();
}

bool SdsSSLSocket::isOpen() {
...
void SdsSSLSocket::close() {
  ...
  if (ssl_ != NULL) {
    ...
    if (rc < 0) {
      if (SSL_get_error(ssl_, rc) != SSL_ERROR_SYSCALL) {
        int errno_copy = THRIFT_GET_SOCKET_ERROR;
        string errors;
        buildErrors(errors, errno_copy);
        throw TSSLException("SSL_shutdown: " + errors);
      }
    }
    SSL_free(ssl_);
    ssl_ = NULL;
...
uint32_t SdsSSLSocket::read(uint8_t* buf, uint32_t len) {
  checkHandshake();
  int32_t bytes = 0;
...
void SdsSSLSocket::write(const uint8_t* buf, uint32_t len) {
  checkHandshake();
...
void SdsSSLSocket::checkHandshake() {
  if (!TSocket::isOpen()) {
...
// SdsSSLSocketFactory implementation
bool     SdsSSLSocketFactory::initialized = false;
uint64_t SdsSSLSocketFactory::count_ = 0;
Mutex    SdsSSLSocketFactory::mutex_;

SdsSSLSocketFactory::SdsSSLSocketFactory(const SSLProtocol& protocol): server_(false) {
  Guard guard(mutex_);
  if (count_ == 0) {
    initializeOpenSSL();
    randomize();
  ...
  SSL_CTX_set_cert_verify_callback(ctx_->get(), always_true_callback, NULL);
}

SdsSSLSocketFactory::~SdsSSLSocketFactory() {
  Guard guard(mutex_);
  ctx_.reset();
  count_--;
  if (count_ == 0) {
...
boost::shared_ptr<SdsSSLSocket> SdsSSLSocketFactory::createSocket() {
  boost::shared_ptr<SdsSSLSocket> ssl(new SdsSSLSocket(ctx_));
  setup(ssl);
  return ssl;
}

boost::shared_ptr<SdsSSLSocket> SdsSSLSocketFactory::createSocket(THRIFT_SOCKET socket) {
  boost::shared_ptr<SdsSSLSocket> ssl(new SdsSSLSocket(ctx_, socket));
  setup(ssl);
  return ssl;
}

boost::shared_ptr<SdsSSLSocket> SdsSSLSocketFactory::createSocket(const string& host,
                                                       int port) {
  boost::shared_ptr<SdsSSLSocket> ssl(new SdsSSLSocket(ctx_, host, port));
  setup(ssl);
  return ssl;
}

void SdsSSLSocketFactory::setup(boost::shared_ptr<SdsSSLSocket> ssl) {
  ssl->server(server());
...
void SdsSSLSocketFactory::ciphers(const string& enable) {
...
void SdsSSLSocketFactory::authenticate(bool required) {
  int mode;
  if (required) {
...
void SdsSSLSocketFactory::loadCertificate(const char* path, const char* format) {
...
void SdsSSLSocketFactory::loadPrivateKey(const char* path, const char* format) {
  if (path == NULL || format == NULL) {
    throw TTransportException(TTransportException::BAD_ARGS,
...
void SdsSSLSocketFactory::loadTrustedCertificates(const char* path) {
  if (path == NULL) {
    throw TTransportException(TTransportException::BAD_ARGS,
...
void SdsSSLSocketFactory::randomize() {
  RAND_poll();
}

void SdsSSLSocketFactory::overrideDefaultPasswordCallback() {
  SSL_CTX_set_default_passwd_cb(ctx_->get(), passwordCallback);
  SSL_CTX_set_default_passwd_cb_userdata(ctx_->get(), this);
}

int SdsSSLSocketFactory::passwordCallback(char* password,
                                        int size,
                                        int,
                                        void* data) {
  SdsSSLSocketFactory* factory = (SdsSSLSocketFactory*)data;
  string userPassword;
...
void SdsSSLSocketFactory::initializeOpenSSL() {
  if (initialized) {
    return;
  }
  initialized = true;
...
void SdsSSLSocketFactory::cleanupOpenSSL() {
  if (!initialized) {
    return;
  }
  initialized = false;
