#ifndef _SDS_SSLSOCKET_H_
#define _SDS_SSLSOCKET_H_
 ...
 */
class SdsSSLSocket: public TSocket {
 public:
 ~SdsSSLSocket();
  /**
...
protected:
   ...
   * Constructor.
   */
  SdsSSLSocket(boost::shared_ptr<SSLContext> ctx);
  /**
   * Constructor, create an instance of SdsSSLSocket given an existing socket.
   *
   * @param socket An existing socket
   */
  SdsSSLSocket(boost::shared_ptr<SSLContext> ctx, THRIFT_SOCKET socket);
  /**
   * Constructor.
   *
...
class SdsSSLSocketFactory {
 public:
   ...
   */
  SdsSSLSocketFactory(const SSLProtocol& protocol = SSLTLS);
  virtual ~SdsSSLSocketFactory();
  /**
   * Create an instance of SdsSSLSocket with a fresh new socket.
   */
  virtual boost::shared_ptr<SdsSSLSocket> createSocket();
  /**
   * Create an instance of SdsSSLSocket with the given socket.
   *
   * @param socket An existing socket.
   */
  virtual boost::shared_ptr<SdsSSLSocket> createSocket(THRIFT_SOCKET socket);
   /**
   * Create an instance of SdsSSLSocket.
   *
   * @param host  Remote host to be connected to
   * @param port  Remote port to be connected to
   */
  virtual boost::shared_ptr<SdsSSLSocket> createSocket(const std::string& host,
                                                     int port);
  /**
 ...
 private:
  ...
  void setup(boost::shared_ptr<SdsSSLSocket> ssl);
 ...
 * It's constructed when application starts and set to SdsSSLSocketFactory
 * instance. It's passed onto all SdsSSLSocket instances created by this factory
 * object.
 */
class AccessManager {
 public:
