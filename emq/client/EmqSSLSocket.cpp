...
// EmqSSLSocket implementation
EmqSSLSocket::EmqSSLSocket(boost::shared_ptr<SSLContext> ctx):
  TSocket(), server_(false), ssl_(NULL), ctx_(ctx) {
}

EmqSSLSocket::EmqSSLSocket(boost::shared_ptr<SSLContext> ctx, THRIFT_SOCKET socket):
  TSocket(socket), server_(false), ssl_(NULL), ctx_(ctx) {
}

EmqSSLSocket::EmqSSLSocket(boost::shared_ptr<SSLContext> ctx, string host, int port):
  TSocket(host, port), server_(false), ssl_(NULL), ctx_(ctx) {
}

EmqSSLSocket::~EmqSSLSocket() {
  close();
}

bool EmqSSLSocket::isOpen() {
...
uint32_t EmqSSLSocket::read(uint8_t* buf, uint32_t len) {
  checkHandshake();
  int32_t bytes = 0;
...
void EmqSSLSocket::write(const uint8_t* buf, uint32_t len) {
  checkHandshake();
...
void EmqSSLSocket::checkHandshake() {
  if (!TSocket::isOpen()) {
...
// EmqSSLSocketFactory implementation
bool     EmqSSLSocketFactory::initialized = false;
uint64_t EmqSSLSocketFactory::count_ = 0;
Mutex    EmqSSLSocketFactory::mutex_;

EmqSSLSocketFactory::EmqSSLSocketFactory(const SSLProtocol& protocol): server_(false) {
  Guard guard(mutex_);
  if (count_ == 0) {
    initializeOpenSSL();
    randomize();
...
EmqSSLSocketFactory::~EmqSSLSocketFactory() {
  Guard guard(mutex_);
  ctx_.reset();
  count_--;
  if (count_ == 0) {
...
boost::shared_ptr<EmqSSLSocket> EmqSSLSocketFactory::createSocket() {
  boost::shared_ptr<EmqSSLSocket> ssl(new EmqSSLSocket(ctx_));
  setup(ssl);
  return ssl;
}

boost::shared_ptr<EmqSSLSocket> EmqSSLSocketFactory::createSocket(THRIFT_SOCKET socket) {
  boost::shared_ptr<EmqSSLSocket> ssl(new EmqSSLSocket(ctx_, socket));
  setup(ssl);
  return ssl;
}

boost::shared_ptr<EmqSSLSocket> EmqSSLSocketFactory::createSocket(const string& host,
                                                       int port) {
  boost::shared_ptr<EmqSSLSocket> ssl(new EmqSSLSocket(ctx_, host, port));
  setup(ssl);
  return ssl;
}

void EmqSSLSocketFactory::setup(boost::shared_ptr<EmqSSLSocket> ssl) {
  ssl->server(server());
...
void EmqSSLSocketFactory::ciphers(const string& enable) {
...
void EmqSSLSocketFactory::authenticate(bool required) {
  int mode;
  if (required) {
...
void EmqSSLSocketFactory::loadCertificate(const char* path, const char* format) {
...
void EmqSSLSocketFactory::loadPrivateKey(const char* path, const char* format) {
  if (path == NULL || format == NULL) {
    throw TTransportException(TTransportException::BAD_ARGS,
...
void EmqSSLSocketFactory::loadTrustedCertificates(const char* path) {
  if (path == NULL) {
    throw TTransportException(TTransportException::BAD_ARGS,
...
void EmqSSLSocketFactory::randomize() {
  RAND_poll();
}

void EmqSSLSocketFactory::overrideDefaultPasswordCallback() {
  SSL_CTX_set_default_passwd_cb(ctx_->get(), passwordCallback);
  SSL_CTX_set_default_passwd_cb_userdata(ctx_->get(), this);
}

int EmqSSLSocketFactory::passwordCallback(char* password,
                                        int size,
                                        int,
                                        void* data) {
  EmqSSLSocketFactory* factory = (EmqSSLSocketFactory*)data;
  string userPassword;
...
void EmqSSLSocketFactory::initializeOpenSSL() {
  if (initialized) {
    return;
  }
  initialized = true;
...
void EmqSSLSocketFactory::cleanupOpenSSL() {
  if (!initialized) {
    return;
  }
  initialized = false;
